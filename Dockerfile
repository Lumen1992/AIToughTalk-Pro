...
RUN mkdir -p /home/appuser/.cache/torch \  
    ...
    && mkdir -p /app/code/models/Lasinya
...
CMD ["python", "-m", "uvicorn", "server:app", "--host", "0.0.0.0", "--port", "9200","--ssl-certfile", "/app/code/certs/127.0.0.1+1.pem","--ssl-keyfile", "/app/code/certs/127.0.0.1+1-key.pem"]
