...
LLM_START_MODEL = "qwen3-235b-a22b"
...
async def health_check():
    ...
    """
    return {"status": "healthy", "service": "aitoughtalk-pro-backend"}
...
if __name__ == "__main__":
    ...
    if not USE_SSL:
        ...
        uvicorn.run("server:app", host="0.0.0.0", port=9200, log_config=None)

    else:
        logger.info("🖥️🔒 Attempting to start server with SSL.")
        # Check if cert files exist
        cert_file = os.getenv("SSL_CERT_FILE", "./certs/127.0.0.1+1.pem")  
        cert_file = "./certs/127.0.0.1+1.pem"
        key_file = os.getenv("SSL_KEY_FILE", "./certs/127.0.0.1+1-key.pem")
