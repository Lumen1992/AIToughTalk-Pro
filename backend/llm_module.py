...
class LLM:
    ...
    """
    SUPPORTED_BACKENDS = ["ollama", "openai", "lmstudio","qwen"]
        ...
        with self._client_init_lock:
            ...
            try:
                if self.backend == "openai":
                    print(self.effective_openai_key, self.effective_openai_base_url)
    ...
    def generate(
        ...
        **kwargs: Any
        ...
        # Catch specific exceptions first
             ...
             logger.error(f"🤖💥 API/Request Error during generation for {req_id}: {e}", exc_info=True)
...
if __name__ == "__main__":
    ...
    if REQUESTS_AVAILABLE:
        try:
            ...
            print("api_key:", os.getenv("QWEN_API_KEY"))
            print("QWEN_API_BASE",os.getenv("QWEN_API_BASE"))
            llm = LLM(
                backend="openai", # Or your backend
            ...
            prewarm_success = llm.prewarm(max_retries=0) # Only one attempt for prewarm after init
            ...
            if prewarm_success:
                 ...
                 inf_time = llm.measure_inference_time(num_tokens=10, temperature=0.1)
                 ...
                 try:
                     ...
                     with LLMGenerationContext(llm, "What is the capital of France? Respond briefly.") as generator:
                     ...
                     direct_gen = llm.generate("List three large cities in Germany.")
